...
import poke_tool.poke_stats_gen_backend.Ninjackalytics_Stat_Queries as nsq
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # if there is no error, then we redirect to the stats page
                ...
                return redirect(
                    url_for("user_navigation.battlestats", bid=redirect_response)
                )
...
def battlestats(bid):
    ...
    for player in pnums:
        ...
        totals[str(player) + " - Total Healing Heal Type Breakdown"] = round(
            healtype["Total"], 2
        )
        ...
        totals[str(player) + " - Total Healing Breakdown"] = round(
            healbrkdwn["Total"], 2
        )
        ...
        totals[str(player) + " - Total Dmg Received Breakdown"] = round(
            dmgrecbrkdwn["Total"], 2
        )
        ...
        totals[str(player) + " - Total Dmg Dealt Breakdown"] = round(
            dmgdealtbrkdwn["Total"], 2
        )
    ...
    return render_template(
        "user_navigation/battlestats.html",
        graphs=graphs,
        user_navigation_info=user_navigation_info,
        bid=bid,
        totals=totals,
    )
