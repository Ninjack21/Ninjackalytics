...
def Get_Team_Info(response):
    ...
    try:
        ...
        if (
            nicknames
            == "Error = Oops! Something went wrong while finding pokemon nickanmes"
            + "\nThe Beheeyem employed have been notified of the error and will take a look!"
        ):
            return (0, nicknames)
...
def Find_nn(log, player_num, mon, battle_id):
    ...
    try:
        ...
        usermsg = (
            "Error = Oops! Something went wrong while finding "
            + str(player_num)
            + "'s "
            + str(mon)
            + "'s nickname"
                ...
                if not "|c|" in line:
                    ...
                    if is_damage == "yes":
                        ...
                        if is_not_move == "no":
                            ...
                            atd_response = Add_To_Damage(
                                dmg_static_vals, dmg_info, nicknames, hp
                            )
                            hp = atd_response[0]
                    ...
                    # now let's check for if the heal keyword is present
                        ...
                        ahi_response = Add_Healing_Info(
                            heal_static_vals, line, turn, nicknames, hp, battle_id
                        )
                        hp = ahi_response[0]
...
def Add_Damage_Info(
    dmg_type, battle_id, turn_num, col_names, table_name, line, turn, nicknames, hp
):
                ...
                if not "|c|" in line:
                    ...
                    if is_switch == "yes":
                        ...
                        if is_move == "yes":
                            ...
                            current_step = (
                                "1 - generate insert information for move switch"
                            )

                            GIS_response = Generate_Insert_Statement(
                            ...
                            funcname = "Generate_Insert_Statement"
                            current_step = (
                                "1 - generate insert information for hard switch"
                            )
